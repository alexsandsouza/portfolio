...
const LEVEL_CONFIG = [
  { min: 90, label: "CYBER MASTER", color: "#FFD700", icon: "👑" },
  { min: 75, label: "ENGENHEIRO SEC", color: "#00E676", icon: "⚡" },
  { min: 60, label: "ANALISTA SOC", color: "#40C4FF", icon: "🔷" },
  { min: 40, label: "SUPORTE JR", color: "#FFB300", icon: "🔶" },
...
// ─── MAIN LEADERBOARD ────────────────────────────────────────────────────────
export default function RankingHDBM11() {
  ...
  // Tick every 10s to update "X min atrás"
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Filter for specific module M11
      loaded = loaded.filter(doc => doc.module === "M11");
  ...
  async function clearAll() {
    try {
      ...
      const deletePromises = snapshot.docs
        .filter(document => document.data().module === "M11")
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ marginTop: 10, fontSize: 12, color: "rgba(255,255,255,0.3)" }}>
            Prof. Alexsander Farias · Aulas 01-04: Segurança em Redes e Equipamentos
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 28 }}>
          {[
            { label: "Analistas Certificados", value: stats.total, icon: "👥", color: "#40C4FF" },
            { label: "Média da Turma", value: stats.total ? `${stats.avg} pts` : "—", icon: "📊", color: "#00E676" },
            { label: "Análises Perfeitas", value: stats.perfect, icon: "💯", color: "#FFD700" },
            { label: "Head de Rede Atual", value: stats.top ? stats.top.name.split(" ")[0] : "—", icon: "🏆", color: "#FF6B9D" },
          ].map(card => (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center" }}>
          ...
          <div style={{ marginLeft: "auto", fontSize: 12, color: "rgba(255,255,255,0.25)", fontFamily: "'Space Mono', monospace" }}>
            {entries.length} {entries.length === 1 ? "analista" : "analistas"}
          </div>
        </div>
        ...
        {entries.length === 0 ? (
          <div style={{
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: "rgba(255,255,255,0.4)", marginBottom: 8 }}>
              Aguardando os primeiros oficiais de segurança...
            </div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.2)" }}>
              Conforme as provas terminarem, os resultados aparecerão aqui na rede.
            </div>
            <div style={{ marginTop: 20, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 12, color: "#40C4FF", fontFamily: "'Space Mono', monospace" }}>Monitorando sub-redes em tempo real...</span>
            </div>
          </div>
        ) : (
          <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
            ...
            <div style={{
              ...
              {["#", "ANALISTA", "PONTUAÇÃO", "TEMPO", "NÍVEL", "ENVIADO"].map((h, i) => (
                <div key={h} style={{
              ...
              {LEVEL_CONFIG.map(lvl => {
                ...
                return (
                  <div key={lvl.label} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 100, fontSize: 11, color: lvl.color, fontFamily: "'Space Mono', monospace", textAlign: "right", flexShrink: 0 }}>
                      {lvl.icon} {lvl.label}
                    </div>
        ...
        <div style={{ textAlign: "center", marginTop: 32, fontSize: 11, color: "rgba(255,255,255,0.15)", fontFamily: "'Space Mono', monospace" }}>
          Sincronização SOC em tempo real · Prof. Alexsander Farias · Hub de Ameaças M11
        </div>
      </div>
      ...
      {showClear && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: "0 0 8px", color: "#fff", fontSize: 18 }}>Apagar base de dados?</h3>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 14, margin: "0 0 24px" }}>
              Todos os resultados do Módulo 11 serão purgados.
            </p>
