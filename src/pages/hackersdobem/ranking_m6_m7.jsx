...
export default function RankingHDBM6M7() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (filter !== "global") {
        loaded = loaded.filter(doc => doc.module === "HDB_M6M7");
      }
  ...
  return (
    <div className="hdb-scanlines" style={{ minHeight: '100vh', background: '#0A0A0F', color: '#fff', padding: '60px 20px', fontFamily: 'var(--hdb-main-font)' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 50 }}>
          <Link to="/hackersdobem/atividade-m6-m7" style={{ color: '#F43F5E', textDecoration: 'none', fontWeight: 700, fontSize: 13 }}>← VOLTAR MISSÃO</Link>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 10, letterSpacing: 5, color: 'rgba(255,255,255,0.3)', marginBottom: 8 }}>HACKERS DO BEM · LEADERBOARD</div>
            <h1 style={{ fontSize: 40, fontWeight: 900, textShadow: '0 0 40px rgba(244,63,94,0.2)' }}>Ranking Móds 06 e 07</h1>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14 }}>Proteção & Backup · Prof. Alexsander Farias · 2026</p>
          </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 15, marginBottom: 40, justifyContent: "center", alignItems: "center" }}>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => setFilter("all")} className="hdb-btn-neon" style={{ padding: "8px 20px", borderRadius: 20, border: "1px solid", borderColor: filter === "all" ? "#F43F5E" : "rgba(255,255,255,0.1)", background: filter === "all" ? "rgba(244,63,94,0.1)" : "transparent", color: filter === "all" ? "#F43F5E" : "#666", cursor: "pointer", fontWeight: 700 }}>M6 & M7</button>
        ...
        <div className="hdb-card" style={{ background: '#13131A', borderRadius: 24, border: '1px solid #2A2A35', overflow: 'hidden', boxShadow: '0 40px 100px rgba(0,0,0,0.6)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr 140px 140px 180px', padding: '25px', background: 'rgba(255,255,255,0.02)', fontWeight: 900, fontSize: 11, letterSpacing: 3, color: 'rgba(255,255,255,0.2)', borderBottom: '1px solid #352A2A' }}>
            ...
            others.map((e, index) => {
              ...
              return (
                <div key={e.id} style={{
                  ...
                  background: isNew ? 'rgba(244,63,94,0.08)' : 'transparent', transition: 'background 0.6s'
                }}>
                  <div style={{ fontSize: 28, fontWeight: 900, color: i === 0 ? '#FFD700' : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : 'rgba(255,255,255,0.1)' }}>{i < 3 ? ["🥇", "🥈", "🥉"][i] : i + 1}</div>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>{e.name} {e.module !== "HDB_M6M7" && <span style={{ fontSize: 10, color: "rgba(64,196,255,0.5)", fontWeight: 400 }}>[{e.module || "OUTRO"}]</span>}</div>
                    <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>Concluído há {timeAgo(e.timestamp)}</div>
                  </div>
                  <div style={{ textAlign: 'center', fontSize: 24, fontWeight: 900, color: '#F43F5E' }}>{e.score}</div>
