...
export default function RankingM01() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (filter !== "global") {
        loaded = loaded.filter(doc => doc.module === "HDB_M01");
      }
    ...
    <div className="hdb-scanlines" style={{ minHeight: '100vh', background: '#0A0A12', color: '#fff', padding: '60px 20px', fontFamily: 'var(--hdb-main-font)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', position: 'relative', zIndex: 10 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 50 }}>
          <Link to="/hackersdobem/atividade-m01" style={{ color: '#00FF88', textDecoration: 'none', fontWeight: 700, fontSize: 13 }}>← VOLTAR MISSÃO</Link>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 10, letterSpacing: 5, color: 'rgba(255,255,255,0.3)', marginBottom: 8 }}>HACKERS DO BEM · LEADERBOARD</div>
            <h1 style={{ fontSize: 40, fontWeight: 900, textShadow: '0 0 40px rgba(0,255,136,0.2)' }}>Ranking Módulo 01</h1>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14 }}>Princípios de Segurança da Informação</p>
          </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 15, marginBottom: 40, justifyContent: "center", alignItems: "center" }}>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => setFilter("all")} className="hdb-btn-neon" style={{ padding: "8px 20px", borderRadius: 20, border: "1px solid", borderColor: filter === "all" ? "#00FF88" : "rgba(255,255,255,0.1)", background: filter === "all" ? "rgba(0,255,136,0.1)" : "transparent", color: filter === "all" ? "#00FF88" : "#666", cursor: "pointer", fontWeight: 700 }}>MÓDULO 01</button>
            ...
            others.map((e, index) => {
              ...
              return (
                <div key={e.id} style={{
                  ...
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>{e.name} {e.module !== "HDB_M01" && <span style={{ fontSize: 10, color: "rgba(64,196,255,0.5)", fontWeight: 400 }}>[{e.module || "OUTRO"}]</span>}</div>
