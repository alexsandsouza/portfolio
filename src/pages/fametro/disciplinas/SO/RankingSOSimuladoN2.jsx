...
const LEVEL_CONFIG = [
  ...
  { min: 70, label: "DISTINÇÃO",  color: "#38BDF8", icon: "⭐" },
...
export default function RankingSOSimuladoN2() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      loaded = loaded.filter(d => d.activityId === "so_simulado_n2");
  ...
  async function clearAll() {
    ...
    try {
      ...
      const dels = snapshot.docs
        .filter(d => d.data().activityId === "so_simulado_n2")
    ...
    <div style={{
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        @keyframes flashBorder { 0%,100%{box-shadow:0 0 0 0 rgba(14,165,233,0);} 50%{box-shadow:0 0 0 6px rgba(14,165,233,0.3);} }
        @keyframes pulse { 50%{opacity:0.4} }
        @keyframes spin { to{transform:rotate(360deg);} }
      `}</style>

      {/* Glow Effects */}
      <div style={{ position: 'fixed', inset: 0, backgroundImage: 'radial-gradient(ellipse at 10% 50%, rgba(14,165,233,0.06) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(56,189,248,0.04) 0%, transparent 50%)', pointerEvents: 'none', zIndex: 0 }} />
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{
              ...
              fontSize: 22, boxShadow: "0 4px 16px rgba(14,165,233,0.3)"
            }}>🏆</div>
            <div>
              <div style={{ fontSize: 9, letterSpacing: 3, color: "#0EA5E9", fontFamily: "monospace", textTransform: 'uppercase' }}>
                FAMETRO · Sistemas Operacionais · Simulado N2
              </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.25)", borderRadius: 20, padding: "5px 12px" }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#0EA5E9", animation: "pulse 1.5s infinite" }} />
              <span style={{ fontSize: 10, fontFamily: "monospace", color: "#0EA5E9", fontWeight: 700 }}>LIVE</span>
            </div>
            {entries.length > 0 && (
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px", position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 28 }}>
          {[
            { label: "Participantes", value: stats.total, icon: "👥", color: "#0EA5E9" },
            ...
            { label: "Líder Atual", value: stats.top ? stats.top.name.split(" ")[0] : "—", icon: "🏆", color: "#38BDF8" },
          ].map(card => (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {[
            ...
            <button key={f.key} onClick={() => setFilterModel(f.key)} style={{
              ...
              border: `1px solid ${filterModel === f.key ? "#0EA5E9" : "rgba(255,255,255,0.08)"}`,
              background: filterModel === f.key ? "rgba(14, 165, 233, 0.15)" : "transparent",
            ...
            {displayed.map((entry, idx) => {
              ...
              return (
                <div key={entry.id} style={{
                  ...
                  background: isNew ? "rgba(14, 165, 233, 0.06)" : "transparent",
                  animation: isNew ? "slideIn 0.5s ease, flashBorder 1.5s ease 2" : "slideIn 0.3s ease",
                  transition: "background 0.3s",
                  alignItems: "center",
                  borderLeft: isNew ? "3px solid #0EA5E9" : isTop3 ? `3px solid ${medalColors[rank - 1]}` : "3px solid transparent"
                }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", gap: 6 }}>
                      {entry.name}
                      {isNew && <span style={{ fontSize: 9, background: "#0EA5E9", color: "#fff", padding: "2px 6px", borderRadius: 10, fontFamily: "monospace" }}>NOVO!</span>}
                    </div>
                    <div style={{ fontSize: 10, color: "#64748B", marginTop: 2 }}>
                      Simulado N2 - Sistemas Operacionais
                    </div>
                  </div>
      ...
      {showClear && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#0D1321", border: "1px solid rgba(14,165,233,0.3)", borderRadius: 16, padding: 28, maxWidth: 360, textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={clearAll} style={{ flex: 1, padding: 12, borderRadius: 10, border: "none", background: "#0EA5E9", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
