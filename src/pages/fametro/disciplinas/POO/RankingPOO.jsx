...
const LEVEL_CONFIG = [
  { min: 90, label: "ARQUITETO SÊNIOR", color: "#FFD700", icon: "👑" },
  { min: 70, label: "DEV PLENO",        color: "#00E676", icon: "⚡" },
  { min: 50, label: "DEV JÚNIOR",       color: "#40C4FF", icon: "🔷" },
  { min: 30, label: "ESTAGIÁRIO",       color: "#FFB300", icon: "🔶" },
...
export default function RankingPOO() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      // Only POO Pilares activity
      loaded = loaded.filter(d => d.activityId === "poo_pilares" || d.module === "POO");
  ...
  async function clearAll() {
    try {
      ...
      const dels = snapshot.docs
        .filter(d => d.data().activityId === "poo_pilares" || d.data().module === "POO")
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(160deg, #060A18 0%, #0A1228 50%, #080E1C 100%)",
      color: "#fff",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @keyframes flashBorder { 0%,100%{box-shadow:0 0 0 0 rgba(59,130,246,0);} 50%{box-shadow:0 0 0 6px rgba(59,130,246,0.4);} }
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{
                ...
                fontSize: 22, boxShadow: "0 4px 16px #3B82F680"
              }}>🎓</div>
              <div>
                <div style={{ fontSize: 10, letterSpacing: 4, color: "#3B82F6", fontFamily: "'Space Mono', monospace" }}>
                  FAMETRO · POO · PILARES DA OO
                </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.25)", borderRadius: 20, padding: "5px 12px" }}>
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#3B82F6", animation: "pulse 1.5s infinite" }} />
                <span style={{ fontSize: 11, fontFamily: "'Space Mono', monospace", color: "#3B82F6", letterSpacing: 1 }}>AO VIVO</span>
              </div>
              {entries.length > 0 && (
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 28 }}>
          {[
            ...
            { label: "Média da Turma", value: stats.total ? `${stats.avg} pts` : "—", icon: "📊", color: "#3B82F6" },
            { label: "Notas Perfeitas", value: stats.perfect, icon: "💯", color: "#FFD700" },
            { label: "Líder Atual", value: stats.top ? stats.top.name.split(" ")[0] : "—", icon: "🏆", color: "#A855F7" },
          ].map(card => (
                ...
                return (
                  <div key={e.id} style={{ flex: 1, maxWidth: 200, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 900, color: podiumColors[i], fontFamily: "'Space Mono', monospace" }}>{e.score}pts</div>
                    ...
                    <div style={{
                      ...
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 28, fontWeight: 900, color: podiumColors[i], fontFamily: "'Space Mono', monospace"
                    }}>
                      {rank}°
                    </div>
                  </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center" }}>
          <div style={{ fontSize: 11, letterSpacing: 3, color: "rgba(255,255,255,0.3)", fontFamily: "'Space Mono', monospace", marginRight: 4 }}>FILTRAR:</div>
          {[{ key: "all", label: "Todos" }, { key: "top", label: "Top 10" }, { key: "recent", label: "Recentes" }].map(f => (
            <button key={f.key} onClick={() => setFilter(f.key)} style={{
              ...
              color: filter === f.key ? "#3B82F6" : "rgba(255,255,255,0.4)",
        ...
        {entries.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 20px", background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.2)" }}>Conforme os alunos finalizarem a atividade, os resultados aparecerão aqui automaticamente.</div>
            <div style={{ marginTop: 20, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 12, color: "#3B82F6", fontFamily: "'Space Mono', monospace" }}>Monitorando em tempo real...</span>
            </div>
          </div>
        ) : (
          <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "52px 1fr 100px 90px 90px 110px", padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.03)" }}>
              {["#", "ALUNO", "PONTUAÇÃO", "TEMPO", "NÍVEL", "ENVIADO"].map((h, i) => (
                <div key={h} style={{ fontSize: 10, letterSpacing: 2, color: "rgba(255,255,255,0.3)", fontFamily: "'Space Mono', monospace", textAlign: i >= 2 ? "center" : "left" }}>{h}</div>
              ))}
            </div>
            ...
            {displayed.map((entry) => {
              ...
              return (
                <div key={entry.id} style={{
                  ...
                  background: isNew ? "rgba(59,130,246,0.06)" : isTop3 ? "rgba(255,255,255,0.02)" : "transparent",
                  ...
                  <div style={{ fontSize: 18, textAlign: "center" }}>
                    {rank <= 3 ? ["🥇", "🥈", "🥉"][rank - 1] : (
                      <span style={{ fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,0.3)", fontFamily: "'Space Mono', monospace" }}>{rank}</span>
                    )}
                  </div>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: isNew ? "#93C5FD" : "#E8EAF6", display: "flex", alignItems: "center", gap: 6 }}>
                      {entry.name}
                      {isNew && <span style={{ fontSize: 10, background: "#3B82F6", color: "#fff", padding: "2px 6px", borderRadius: 10, fontFamily: "'Space Mono', monospace", letterSpacing: 1 }}>NOVO!</span>}
                    </div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", marginTop: 2 }}>
                      {entry.activityId === "poo_pilares" ? "POO - Pilares da OO" : "POO"}
                    </div>
                  </div>
                  <div style={{ textAlign: "center" }}>
                    <div style={{
                      fontSize: 20, fontWeight: 900,
                      color: entry.score >= 90 ? "#FFD700" : entry.score >= 70 ? "#00E676" : entry.score >= 50 ? "#40C4FF" : entry.score >= 30 ? "#FFB300" : "#FF5252",
                  ...
                  <div style={{ textAlign: "center" }}>
                    <div style={{
                      ...
                      color: lvl.color, fontFamily: "'Space Mono', monospace", letterSpacing: 0.5
                    }}>
                      {lvl.icon} {lvl.label}
                    </div>
                  </div>
        ...
        {entries.length > 0 && (
          <div style={{ marginTop: 28, background: "rgba(255,255,255,0.02)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)", padding: 20 }}>
            <div style={{ fontSize: 11, letterSpacing: 3, color: "rgba(255,255,255,0.3)", fontFamily: "'Space Mono', monospace", marginBottom: 16 }}>DISTRIBUIÇÃO DE NÍVEIS</div>
              ...
              {LEVEL_CONFIG.map(lvl => {
                ...
                return (
                  <div key={lvl.label} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 110, fontSize: 11, color: lvl.color, fontFamily: "'Space Mono', monospace", textAlign: "right", flexShrink: 0 }}>
                      {lvl.icon} {lvl.label}
                    </div>
                    ...
                    <div style={{ width: 30, fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.5)", fontFamily: "'Space Mono', monospace" }}>{count}</div>
                  </div>
                );
              })}
            </div>
      ...
      {showClear && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#0D1330", border: "1px solid rgba(255,82,82,0.3)", borderRadius: 16, padding: 28, maxWidth: 360, textAlign: "center" }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>⚠️</div>
            <h3 style={{ margin: "0 0 8px", color: "#fff", fontSize: 18 }}>Limpar ranking?</h3>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 14, margin: "0 0 24px" }}>Todos os resultados dos alunos serão apagados permanentemente.</p>
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={clearAll} style={{ flex: 1, padding: 12, borderRadius: 10, border: "none", background: "#FF5252", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
