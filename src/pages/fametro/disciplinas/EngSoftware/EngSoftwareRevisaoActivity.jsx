...
const STUDY_CASE_A = {
  title: "Refatoração Arquitetural do Sistema MedTrack",
  context: "O \"MedTrack\" é um sistema de rastreamento de medicamentos controlados utilizado por uma rede de 200 farmácias no estado do Amazonas. Desenvolvido há 12 anos em PHP monolítico, o sistema processa diariamente mais de 50.000 transações de dispensação de medicamentos e interage com o Sistema Nacional de Gerenciamento de Produtos Controlados (SNGPC) da ANVISA.\n\nNos últimos 3 anos, a equipe de desenvolvimento tem enfrentado sérios problemas: cada nova funcionalidade exigida pela ANVISA leva em média 4 meses para ser implementada (quando o prazo regulatório é de 60 dias). Testes manuais consomem 70% do tempo do ciclo de entrega. O código possui mais de 800 mil linhas sem cobertura de testes automatizados, e uma alteração no módulo de dispensação recentemente corrompeu o módulo de relatórios fiscais, gerando multas de R$ 2 milhões.\n\nO novo CTO contratado propôs uma estratégia de modernização em três frentes: (1) adoção imediata de TDD para novos módulos, (2) encapsulamento do núcleo legado com APIs RESTful, e (3) implantação de pipeline CI/CD com Jenkins. A diretoria questiona se o investimento se justifica, dado que o sistema \"ainda funciona\".",
  statement: "Com base no cenário apresentado, elabore uma análise dissertativa-argumentativa abordando os seguintes pontos:\n\n1) Identifique e explique quais Leis de Lehman estão se manifestando no MedTrack e como elas justificam a necessidade de modernização.\n2) Classifique os três tipos de manutenção que o sistema vem sofrendo nos últimos anos, justificando cada classificação.\n3) Argumente por que a estratégia proposta pelo CTO (TDD + Wrapping + CI/CD) é tecnicamente superior à abordagem Big Bang de reescrita total.\n4) Explique como o TDD pode melhorar a arquitetura do sistema e reduzir o acoplamento entre módulos.",
  criteria: "DIRETRIZES DE RESPOSTA ESPERADA:\n\n1) Leis de Lehman:\n• Lei da Mudança Contínua: o sistema precisa evoluir para atender às regulamentações da ANVISA ou se torna obsoleto.\n• Lei da Complexidade Crescente: sem refatoração proativa, 12 anos de patches degradaram a estrutura (módulo de dispensação afetando relatórios fiscais).\n\n2) Tipos de Manutenção:\n• Adaptativa: adequações às novas exigências regulatórias da ANVISA/SNGPC.\n• Corretiva: correção do bug que corrompeu o módulo de relatórios fiscais.\n• Perfectiva: otimizações de performance e melhorias estruturais internas.\n\n3) Superioridade da estratégia evolutiva:\n• Wrapping (encapsulamento) preserva o núcleo funcional enquanto expõe APIs modernas, reduzindo risco operacional.\n• Big Bang é inaceitável para sistema crítico de saúde pública com 200 farmácias dependentes.\n• CI/CD automatiza testes e entregas, reduzindo o ciclo de 4 meses para dias.\n\n4) TDD e arquitetura:\n• Escrever testes antes do código força interfaces desacopladas e módulos coesos.\n• Garante cobertura de testes desde a concepção, eliminando os 70% de tempo gasto em testes manuais.\n• O ciclo Red-Green-Refactor promove refatoração contínua e design emergente."
};

const STUDY_CASE_B = {
  title: "Evolução do Sistema EduConnect para Ensino Híbrido",
  context: "O \"EduConnect\" é uma plataforma de gestão acadêmica utilizada por 15 universidades particulares no Norte do Brasil, processando matrículas, notas e frequências de mais de 120.000 alunos. O sistema foi originalmente desenvolvido em 2013 como uma aplicação desktop Java Swing, posteriormente adaptada para web com JSP.\n\nCom a pandemia e a consolidação do ensino híbrido, o Ministério da Educação passou a exigir integração com ambientes virtuais de aprendizagem (AVA), emissão de diplomas digitais e dashboards de acompanhamento de evasão em tempo real. A equipe de TI, composta por 4 desenvolvedores, tenta implementar essas funcionalidades diretamente no código legado, mas cada sprint gera em média 15 bugs de regressão.\n\nOs testes são executados manualmente por um único analista de QA, que leva 3 semanas para validar uma release. O deploy é feito por FTP em um servidor compartilhado, e já houve dois incidentes graves onde versões incorretas foram enviadas para produção, deixando o sistema de matrículas fora do ar por 48 horas em período de rematrícula.\n\nO conselho administrativo aprovou orçamento para modernização e contratou uma consultoria que recomendou: (1) migração gradual para microsserviços com encapsulamento do legado, (2) implementação de esteira CI/CD com GitLab CI e Docker, e (3) adoção de TDD e testes de aceitação automatizados para os novos módulos.",
  statement: "Com base no cenário do EduConnect, elabore uma análise dissertativa-argumentativa abordando:\n\n1) Identifique qual Lei de Lehman explica a crescente dificuldade de evolução do EduConnect e por que o congelamento tecnológico não é uma opção viável.\n2) Explique a diferença entre Verificação e Validação (V&V) e como a ausência dessas práticas contribuiu para os incidentes de deploy incorreto.\n3) Classifique os quatro níveis de teste de software e indique quais níveis estão ausentes ou deficientes no processo atual do EduConnect.\n4) Argumente como a adoção de CI/CD resolve os problemas de deploy e por que a estratégia de modernização gradual é preferível à substituição completa do sistema.",
  criteria: "DIRETRIZES DE RESPOSTA ESPERADA:\n\n1) Lei de Lehman aplicável:\n• Lei da Mudança Contínua: o EduConnect precisa evoluir continuamente para atender às novas exigências do MEC (ensino híbrido, diplomas digitais, dashboards de evasão), ou se tornará progressivamente inútil.\n• O congelamento levaria ao abandono por soluções paralelas.\n\n2) Verificação vs. Validação:\n• Verificação: \"estamos construindo o produto corretamente?\" — ausente, pois não há testes automatizados para garantir conformidade com especificações.\n• Validação: \"estamos construindo o produto certo?\" — comprometida, pois os deploys incorretos indicam que o produto entregue não corresponde ao esperado.\n• Os incidentes de FTP mostram falha grave tanto na verificação (código não testado) quanto no processo de entrega.\n\n3) Níveis de teste:\n• Teste de Unidade: ausente — sem testes automatizados para funções individuais.\n• Teste de Integração: deficiente — bugs de regressão entre módulos indicam falha na verificação de interfaces.\n• Teste de Sistema: parcialmente manual — o analista de QA faz isso em 3 semanas.\n• Teste de Aceitação: ausente — não há validação formal com os coordenadores acadêmicos.\n\n4) CI/CD e modernização gradual:\n• CI/CD elimina deploys manuais por FTP, automatizando build, testes e entrega com pipelines reproduzíveis.\n• Integração contínua detecta bugs de regressão imediatamente, não após 3 semanas.\n• Modernização gradual (wrapping + microsserviços) preserva a operação das 15 universidades enquanto novos módulos são desenvolvidos com qualidade.\n• Substituição Big Bang é inaceitável: 120.000 alunos dependem do sistema diariamente."
};
