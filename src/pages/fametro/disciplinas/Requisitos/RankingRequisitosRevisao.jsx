...
const LEVEL_CONFIG = [
  ...
  { min: 75, label: "DISTINÇÃO",  color: "#A78BFA", icon: "⭐" },
  { min: 60, label: "APROVADO",   color: "#3B82F6", icon: "✅" },
...
export default function RankingRequisitosRevisao() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      loaded = loaded.filter(d => d.activityId === "requisitos_revisao");
  ...
  async function clearAll() {
    try {
      ...
      const dels = snapshot.docs
        .filter(d => d.data().activityId === "requisitos_revisao")
    ...
    <div style={{
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        @keyframes flashBorder { 0%,100%{box-shadow:0 0 0 0 rgba(239,68,68,0);} 50%{box-shadow:0 0 0 6px rgba(239,68,68,0.3);} }
        @keyframes pulse { 50%{opacity:0.4} }
        @keyframes spin { to{transform:rotate(360deg);} }
      `}</style>

      {/* Background Effect */}
      <div style={{ position: 'fixed', inset: 0, backgroundImage: 'radial-gradient(ellipse at 10% 50%, rgba(239,68,68,0.06) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(59,130,246,0.04) 0%, transparent 50%)', pointerEvents: 'none', zIndex: 0 }} />
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{
              ...
              fontSize: 22, boxShadow: "0 4px 16px rgba(239,68,68,0.3)"
            }}>🏆</div>
            <div>
              <div style={{ fontSize: 9, letterSpacing: 3, color: "#EF4444", fontFamily: "monospace", textTransform: 'uppercase' }}>
                FAMETRO · Requisitos e Projetos · 5º Período
              </div>
              <h1 style={{ margin: "4px 0 0", fontSize: 20, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>
                Revisão Avaliação — Placar ao Vivo
              </h1>
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 20, padding: "5px 12px" }}>
              ...
              <span style={{ fontSize: 10, fontFamily: "monospace", color: "#EF4444", fontWeight: 700 }}>LIVE</span>
            </div>
            {entries.length > 0 && (
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px", position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 28 }}>
          {[
            { label: "Participantes", value: stats.total, icon: "👥", color: "#EF4444" },
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {[
            ...
            <button key={f.key} onClick={() => setFilterModel(f.key)} style={{
              ...
              border: `1px solid ${filterModel === f.key ? "#EF4444" : "rgba(255,255,255,0.08)"}`,
              background: filterModel === f.key ? "rgba(239,68,68,0.15)" : "transparent",
            ...
            {displayed.map((entry, idx) => {
              ...
              return (
                <div key={entry.id} style={{
                  ...
                  alignItems: "center",
                  borderLeft: isNew ? "3px solid #EF4444" : isTop3 ? `3px solid ${medalColors[rank - 1]}` : "3px solid transparent"
                }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", gap: 6 }}>
                      {entry.name}
                      {isNew && <span style={{ fontSize: 9, background: "#EF4444", color: "#fff", padding: "2px 6px", borderRadius: 10, fontFamily: "monospace" }}>NOVO!</span>}
                    </div>
      ...
      {showClear && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#0D1321", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 16, padding: 28, maxWidth: 360, textAlign: "center" }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>⚠️</div>
            <h3 style={{ margin: "0 0 8px", color: "#fff", fontSize: 18 }}>Limpar ranking?</h3>
            <p style={{ color: "#94A3B8", fontSize: 13, margin: "0 0 24px", lineHeight: 1.5 }}>Todos os resultados dos alunos da Atividade de Revisão serão apagados permanentemente do Firestore.</p>
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={clearAll} style={{ flex: 1, padding: 12, borderRadius: 10, border: "none", background: "#EF4444", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
